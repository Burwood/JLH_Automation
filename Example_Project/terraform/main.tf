...
# "ddc" represents a locally significant name to identify this resource elsewhere 
...
resource "vsphere_virtual_machine" "ddc" {
  ...
  # this will call the variable "vm_ddc" which has the hostname prefix of 
  # "ddc-ctx-lab-0" in lab.tfvars.  Count index starts at "0", so we will 
  # add 1 to get vm01, vm02 etc. instaead of vm00 vm01... 
...
# same stuff here, except we are giving this resrouce the name "storefront"
# and calling a different variable for generating it's inventory name and hostname
...
# Since we are only creating 1 vm in this resoruce block, it was chosen to 
# have the full VM name be defined in the variable and omit the count arguement.  
# Optionally, we could have used a similar format as the preceeding resource 
# blocks and just given it a count value of 1.  
# The VDA resource block below illustrates just that.
