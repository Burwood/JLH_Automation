...
resource "vsphere_folder" "folder" {
  ...
  provisioner "local-exec" {
    command    = "echo 'vSphere Folder: ${self.vsphere_folder}\n\n| **Hostname** | **vCPU** | **RAM** | **Disk** |\n| --- | --- | --- | --- |' >> tf_info.md"
    on_failure  = continue
  }
}
...
resource "vsphere_virtual_machine" "ddc" {
  ...
  clone {
    ...
    customize {
      windows_options {
        ...
        domain_admin_user     = var.cvad_lab_domain_user
        domain_admin_password = var.cvad_lab_domain_pass
  ...
  provisioner "local-exec" {
    command    = "echo '| ${self.name} | ${self.num_cpus} | ${self.memory} | ${self.disk.0.size} |' >> tf_info.md"
    on_failure = continue
  }
}
